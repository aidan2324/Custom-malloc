/*
 * Aidan Murray (32251885) and Cole Feely (32237741)
 */
...
void eval(char *cmdline)
{
    //creates array for parsline to fill
    char *argv[MAXARGS];
    pid_t pid;
    sigset_t mask;
    sigset_t prev_mask;
    // background or foreground is decided
    int bg = parseline(cmdline, argv);
    if ( argv[0] == NULL){return;}; // check if valid builtin builtin_cmd
    if(!builtin_cmd(argv)){
        // if not a user command
        sigemptyset(&mask);
        sigaddset(&mask, SIGCHLD);
        sigprocmask(SIG_BLOCK, &mask, &prev_mask);
        // fork
        if((pid = fork()) < 0){unix_error("ERROR: unable to run command");}
        else if(pid == 0){ //if process is the child
            ...
            if(!bg){
                if(addjob(jobs, pid, FG, cmdline)){//add job
                    sigprocmask(SIG_SETMASK, &prev_mask, NULL);
                    waitfg(pid);}
                else{kill(-pid, SIGINT);}//if job doesnt work kill the child
            }
            else{
               addjob(jobs, pid, BG, cmdline);
               printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline);
               sigprocmask(SIG_SETMASK, &prev_mask, NULL); //allow parient to receive sigchild
            }
        }
    }
}
...
void sigint_handler(int sig)
{
    ...
    if (pid) {kill(-pid, SIGINT);} //if pid is valid
    return;
}
...
{
    pid_t pid = fgpid(jobs);
    if (pid) {kill(-pid, SIGTSTP);} //if pid is valid 
    return;
}
