...
void* mm_malloc (size_t size) {
  ...
  if (size <= MIN_BLOCK_SIZE) {
		// set a var for the min block size
  ...
  if(ptrFreeBlock== NULL){
	...
	// if there is no free block, get more space
	requestMoreSpace(reqSize);

	// Set var for proced
	prevBlockTag=TAG_PRECEDING_USED;
	// Set pointer of the free block
	ptrFreeBlock = searchFreeList(reqSize);	
  }
  else {
	prevBlockTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
	}

  blockSize = SIZE(ptrFreeBlock->sizeAndTags);
  removeFreeBlock(ptrFreeBlock);

  int actSize = blockSize - reqSize;
  // if actual block size is up to standard
  if( actSize > MIN_BLOCK_SIZE){
	
	// Set pointer to free block
	ptrFreeBlock->sizeAndTags = reqSize | prevBlockTag;
	// Use our temp block
	newBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
	// Update the new block
	newBlock->sizeAndTags = ((actSize) | TAG_PRECEDING_USED) & (~TAG_USED);
	

	// set pointers to actual and req Sizes
	*((int*)UNSCALED_POINTER_ADD(ptrFreeBlock, (blockSize-WORD_SIZE))) = actSize;
	*((int*)UNSCALED_POINTER_ADD(ptrFreeBlock, (reqSize-WORD_SIZE)))=reqSize;

	// insert into the free blocks, the new block
	insertFreeBlock(newBlock);
  }
  // Else if not up to standard, update the previous block and tags used
  else {
	// Set up next block
	BlockInfo* nextBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);

	// Set pointer as the block size
	*((int*)UNSCALED_POINTER_ADD(ptrFreeBlock, (blockSize-WORD_SIZE)))=blockSize;

	// update next block size and tags
	nextBlock->sizeAndTags = nextBlock->sizeAndTags | TAG_PRECEDING_USED;
  }

  // Signal tag as T
  ptrFreeBlock->sizeAndTags =  ptrFreeBlock->sizeAndTags | prevBlockTag;
  // Remove from free
  ptrFreeBlock->sizeAndTags =  ptrFreeBlock->sizeAndTags | TAG_USED;


  return (
	(void*) UNSCALED_POINTER_ADD(ptrFreeBlock, WORD_SIZE)
	);
...
void mm_free (void *ptr) {
	// add one word of space to account for the header
	BlockInfo * blockInfo = (BlockInfo*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

	//check if block already freed. do nothing if so
	if(((blockInfo->sizeAndTags) & TAG_USED)==0){return;}

	//filter out last byte (all bits are 1 except last in ~(0 << 1)
	blockInfo->sizeAndTags &= ~(0 << 1);

	int size = SIZE(blockInfo->sizeAndTags); //get size
	//next block is the size of this block plus this blocks tags away
	BlockInfo * nextBlock = (BlockInfo*)UNSCALED_POINTER_ADD(blockInfo, size);
	//set next blocks size and tags
	nextBlock->sizeAndTags = nextBlock->sizeAndTags & (~TAG_PRECEDING_USED);
...
// Extra credit.
void *mm_realloc(void *old_bp, size_t new_size)
{
	if(old_bp == NULL){return mm_malloc(new_size);}
	if(new_size == 0){
		mm_free(old_bp);
		return NULL;
	}
	void *new_bp;
	size_t old_size;
	BlockInfo * old_BlockInfo;
	//create new block, it will be identical to old block until the sma new or old block
	new_bp = mm_malloc(new_size);
	old_BlockInfo = (BlockInfo *) UNSCALED_POINTER_SUB(old_bp, WORD_SIZE);
	old_size = SIZE(old_BlockInfo->sizeAndTags);
	//this is so that if the new block is smaller than old its still same size as old but 
	//anything past the size of the new will be unitialized
	if (new_size < old_size){old_size = new_size;}
	//copy the old block to where the new block is then free the old block
	memcpy(new_bp, old_bp, old_size);
	mm_free(old_bp);
	//old size ends up bring the size of the new unless new is smaller than old
	return new_bp;
}
